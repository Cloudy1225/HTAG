...
from train_and_test import train_mlp, test_mlp
...
def run(args):
    ...
    model = MLP(in_dim, conf['hid_dims'], num_classes, conf['dropout'], conf['activation'], conf['norm_type'])
    ...
    for epoch in range(1, conf['epochs'] + 1):
        ...
        loss = train_mlp(model, feats_train, labels_train, batch_size, optimizer, device, multilabel)
        time_taken = str(datetime.timedelta(seconds=(time.time() - epoch_start)))
        if epoch % conf['eval_interval'] == 0:
            micro_val, macro_val = test_mlp(model, feats_val, labels_val, batch_size, device, multilabel)
    ...
    micro_test, macro_test = test_mlp(model, feats_test, labels_test, batch_size, device, multilabel)
